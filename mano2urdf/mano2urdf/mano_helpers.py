...
def get_limit(is_rhand=True):
    ...
    if is_rhand:
        return [rhand_lower_limit, rhand_upper_limit]
    else:
...
def get_mano_data(model_path, is_rhand=True, ncomps=24, v_template=None):
    ...
    # smplx returns 16 MANO joints (wrist + 15); append 5 fingertips picked from
    # vertices so joints_dict below has the 21 names it expects.
    if joints.shape[0] == 16:
        if is_rhand:
            tips = verts[[745, 317, 444, 556, 673]]
        else:
            tips = verts[[745, 317, 445, 556, 673]]
        joints = np.concatenate((joints, tips))
