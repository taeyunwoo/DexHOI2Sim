# Retrieved from ArtiGrasp (https://github.com/zdchan/artigrasp, rsc/mano2urdf/). Locally modified.
import os.path
import json
import shutil
...
def trimesh_to_pymeshlab_mesh(tm: trimesh.Trimesh) -> Mesh:
    v = np.ascontiguousarray(np.asarray(tm.vertices, dtype=np.float64))
    f = np.ascontiguousarray(np.asarray(tm.faces,    dtype=np.int32))
    ...
    if v.size == 0 or f.size == 0:
        raise ValueError("Empty mesh: vertices or faces are empty.")

    if (f < 0).any() or (f >= len(v)).any():
        tm.remove_unreferenced_vertices()
        v = np.ascontiguousarray(np.asarray(tm.vertices, dtype=np.float64))
        f = np.ascontiguousarray(np.asarray(tm.faces,    dtype=np.int32))

    return Mesh(vertex_matrix=v, face_matrix=f)

def decimate_with_pymeshlab(tm: trimesh.Trimesh, decimation_factor: float) -> trimesh.Trimesh:
    target = int(len(tm.faces) * decimation_factor)
    if target < 4:
        return tm.copy()

    ms = MeshSet()
    in_mesh = trimesh_to_pymeshlab_mesh(tm)
    ms.add_mesh(in_mesh, "in")
    ...
    out = ms.current_mesh()
    v = out.vertex_matrix()  # float64 (N,3)
    f = out.face_matrix()    # int32  (M,3)
...
def process_mesh_with_trimesh(verts, density, show=False):
    ...
    cyl = getattr(bounding_primitive, "primitive", bounding_primitive)
    ...
    transform = np.array(cyl.transform, dtype=np.float64, copy=True)
...
def export_body_part_meshes(out_path, lbs_weight_matrix, vertices, joint_names,
    ...
    for idx, name in enumerate(joint_names):
        argmax_idxs = np.empty(0, dtype=np.int64)
        ...
        if decimation_factor_obj != 1.0:
            ...
            # ms.meshing_decimation_quadric_edge_collapse(targetfacenum=int(n_faces * decimation_factor_obj))

            # ms = ms.simplify_quadratic_decimation(
            #     face_count=ms.area_faces.size * decimation_factor_obj)

            # ty
            ms = decimate_with_pymeshlab(ms, decimation_factor_obj)
...
def ty_get_palm(out_path, lbs_weight_matrix, vertices, joint_names,
                            decimation_factor_obj=1.0,
                            density=980,  # kg/m^3
                            is_rhand=True):
    argmax_lbs_matrix = np.argmax(lbs_weight_matrix, axis=-1)
...
def get_obj_data(model_path, out_path, density=980): # density in kg/m^3
    model_path_top = os.path.join(model_path, 'top.obj')
    model_path_bottom = os.path.join(model_path, 'bottom.obj')
    # param_path = os.path.join(model_path, 'object_params.json')
    ...
    # assert os.path.exists(param_path), f"Not found: {param_path}"

    # copy the mesh file to workspace directory 
    #shutil.copyfile(model_path_top, "/home/luocheng/raisim_grasp/raisim_grasp_arctic/rsc/arctic/top_watertight_tiny.obj")
    #shutil.copyfile(model_path_bottom, "/home/luocheng/raisim_grasp/raisim_grasp_arctic/rsc/arctic/bottom_watertight_tiny.obj")
    ...
    # vertices_t = mesh_t.vertices / 1000
    # vertices_b = mesh_b.vertices / 1000
    ...
    # mesh_t_meter.density = density
    # mesh_b_meter.density = density
    mesh_t_meter.density = density * 10
    mesh_b_meter.density = density * 10
    ...
    _save_mesh(mesh_t_meter, out_path, name_t + ".obj")
    _save_mesh(mesh_t_meter, out_path, name_t + ".stl")
    _save_mesh(mesh_b_meter, out_path, name_b + ".obj")
    _save_mesh(mesh_b_meter, out_path, name_b + ".stl")
